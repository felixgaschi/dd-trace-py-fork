...
# AIDEV-NOTE: libddwaf 2.0 subcontext ownership. RASP evaluations run on a subcontext (one per
# guarded operation) instead of v1 ephemeral data. SSRF needs a single subcontext shared across
# an outgoing request's SSRF_REQ + SSRF_RES calls, which may run in different core execution
# contexts (e.g. httpx fires SSRF_REQ on the inner send context and the final SSRF_RES on the
# outer request context). We store a mutable holder on the owning context so both inner and outer
# calls resolve the same subcontext via core.find_item (which walks up the context stack). The
# holder is dropped when the owning scope ends, releasing the subcontext capsule (whose __del__
# destroys the subcontext).
...
def open_rasp_subcontext_scope() -> bool:
    """Establish the current core context as the owner of an SSRF subcontext.

    Called by SSRF integrations at the start of the scope that bounds a single outgoing request,
    so that the request's SSRF_REQ and SSRF_RES WAF calls share one subcontext. The subcontext
    itself is created lazily on the first RASP call within the scope.

    Reentrant: if an enclosing scope already owns a holder (e.g. ``requests`` wrapping
    ``urllib3``), this is a no-op and the inner operation shares the outer subcontext. Returns
    ``True`` only when this call created the holder, so the caller knows whether it must close it.
    """
    if core.find_item(_RASP_SUBCONTEXT) is not None:
        return False
    core.set_item(_RASP_SUBCONTEXT, _RaspSubcontextHolder())
    return True


def close_rasp_subcontext_scope(opened: bool = True) -> None:
    """Release an SSRF subcontext scope opened by ``open_rasp_subcontext_scope``.

    Used by integrations that open the scope without a dedicated ``core.context_with_data`` block
    (e.g. urllib3, where the request and response WAF calls happen in one function). Pass the
    value returned by ``open_rasp_subcontext_scope`` so a no-op open (an enclosing scope already
    owned the holder) does not clear the parent's holder. Integrations that own the scope through
    a context manager don't need this call: dropping the context releases the holder.
    """
    if opened:
        core.set_item(_RASP_SUBCONTEXT, None)
...
def get_or_create_rasp_subcontext(ddwaf: Any, ctx: Any, rule_type: Optional[str]) -> Optional[Any]:
    ...
    subcontext could be created, in which case the caller falls back to the main context.
    """
