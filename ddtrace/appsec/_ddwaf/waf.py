...
class DDWaf:
    ...
    def __init__(
        ...
        # libddwaf 2.0 removed ddwaf_config: the obfuscator regexes are now supplied as a builder
        # configuration. Both keys are optional and libddwaf falls back to its built-in defaults
        # when omitted, so we only add the config when at least one regex is provided. The builder
        # copies the config, so the source object can be released right after adding it.
        obfuscator = {}
    ...
    def new_subcontext(self, ctx: ddwaf_context_capsule) -> Optional[ddwaf_subcontext_capsule]:
        ...
        subcontext per guarded operation).
        """
        if not ctx:
            return None
    ...
    def run(
        ...
        # libddwaf 2.0 evaluates a single data object per call (ephemeral data is no longer a
        # separate parameter). The main context handles request-related data while subcontexts
        # handle non-persisting RASP data; in both cases persistent and former-ephemeral
        # addresses are merged into one map. Address names never collide between the two.
        merged: DDWafRulesType
        if ephemeral_data:
            if isinstance(data, dict) and isinstance(ephemeral_data, dict):
                merged = {**data, **ephemeral_data}
            else:
                merged = ephemeral_data
        else:
            merged = data

        result_obj = ddwaf_object()
        observator = _observator()
        wrapper = ddwaf_object(merged, observator=observator)
        if is_subcontext:
            subctx = cast(ddwaf_subcontext_capsule, target)
            handle, lock = subctx.subctx, subctx._lock
            eval_fn = ddwaf_subcontext_eval
        else:
            ctx = cast(ddwaf_context_capsule, target)
            handle, lock = ctx.ctx, ctx._lock
            eval_fn = ddwaf_context_eval
        with lock:
            error = eval_fn(handle, wrapper, DEFAULT_ALLOCATOR, result_obj, int(timeout_ms * 1000))
        # Input-data ownership after eval (see ddwaf.h ddwaf_context_eval):
        #   - DDWAF_OK / DDWAF_MATCH: the context took ownership; do not free the wrapper here
        #     (it is freed on context/subcontext destruction).
        #   - DDWAF_ERR_INVALID_OBJECT (-2): libddwaf already freed the input; must not read or
        #     free the wrapper (reading wrapper.struct would be a use-after-free).
        #   - DDWAF_ERR_INVALID_ARGUMENT (-1): the input was not freed and not taken; free it here.
        #   - DDWAF_ERR_INTERNAL (-3): ownership is undefined; leave the wrapper alone.
        if error < 0:
            # Log the source python structure, never wrapper.struct (the wrapper may be freed).
            LOGGER.debug("run DDWAF error: %d\ninput %s\nerror %s", error, merged, self.info.errors)
