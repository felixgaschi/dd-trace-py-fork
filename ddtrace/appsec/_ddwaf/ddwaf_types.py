...
# libddwaf 2.0 caps containers at uint16; pass a capacity hint within that range
# (libddwaf grows the container dynamically when the real size is larger).
DDWAF_OBJ_MAX_CAPACITY = 0xFFFF


# AIDEV-NOTE: libddwaf 2.0 type values are NOT a simple bitfield — DDWAF_OBJ_UNSIGNED (0x06)
# overlaps DDWAF_OBJ_BOOL (0x02) | DDWAF_OBJ_SIGNED (0x04). Always compare with exact equality,
# never with bitmasks. There are three string variants (heap, literal, and inline "small").
...
class ddwaf_object(ctypes.Union):
    ...
    def from_json_bytes(cls, data: bytes) -> Optional["ddwaf_object"]:
        ...
        if not ddwaf_object_from_json(obj, data, len(data), DEFAULT_ALLOCATOR):
            ...
            # Free any partial allocation left by the parser (no-op when nothing was allocated:
            # the zero-initialised object has type DDWAF_OBJ_INVALID and destroy is idempotent).
    ...
    def struct(self) -> DDWafRulesType:
        ...
        if t == DDWAF_OBJ_TYPE.DDWAF_OBJ_SMALL_STRING:
            ...
            if not sstr.size:
                return ""
            # Read length-based: slicing the c_char array would truncate at the first embedded
            # NUL byte, corrupting strings that legitimately contain NUL (e.g. binary input).
...
# Both forms are accepted by the ``(1, "object")`` argument of the set_* functions.
#
...
def _build_sequence(
    ...
    if max_depth <= 0:
        ...
        # libddwaf 2.0 stores the container size in a uint16, so it cannot represent more than
        # DDWAF_OBJ_MAX_CAPACITY elements; building past it would silently wrap the size field.
        # Cap here (even on the no-limit path) and record the truncation, mirroring libddwaf.
        max_objects = DDWAF_OBJ_MAX_CAPACITY
    ddwaf_object_set_array(self, min(len(struct), DDWAF_OBJ_MAX_CAPACITY), DEFAULT_ALLOCATOR)
...
def _build_mapping(
    ...
    if max_depth <= 0:
        ...
        # libddwaf 2.0 stores the container size in a uint16; cap here (even on the no-limit path)
        # to avoid silently wrapping the size field, and record the truncation.
        max_objects = DDWAF_OBJ_MAX_CAPACITY
    ddwaf_object_set_map(self, min(len(struct), DDWAF_OBJ_MAX_CAPACITY), DEFAULT_ALLOCATOR)
