...
class ddwaf_context_capsule(Generic[T]):
    def __init__(self, ctx: type[T], free_fn: Callable[[type[T]], None]) -> None:
        ...
        # AIDEV-NOTE: This lock serializes concurrent ddwaf_context_eval calls on the same context.
...
class ddwaf_subcontext_capsule(Generic[T]):
    # AIDEV-NOTE: libddwaf 2.0 replaced ephemeral data with subcontexts. A subcontext inherits
    # the parent context's persistent data but evaluates non-persisting data (RASP, one per
    # guarded operation). It carries its own lock for the same thread-safety reason as
    # ddwaf_context_capsule: ddwaf_subcontext_eval / ddwaf_subcontext_destroy must not run
    # concurrently on the same subcontext.
